...
This module has NO idapro/IDAPython dependency on purpose: the idalib
supervisor (which must never import idapro) reads this registry to enumerate
and adopt worker/GUI instances. User-dir resolution is therefore based only
on environment variables, with an explicit IDA_MCP_USER_DIR override.

N-copies note: each spawned worker registers under its own ephemeral PORT, so
N duplicate workers of the SAME binary produce N distinct instance files with
the same idb_path but different port/pid/session_id. Path-based lookup is
therefore ambiguous for duplicates; prefer session_id-keyed lookup when a
caller wants to attach to a specific copy.
"""
...
def _get_ida_user_dir() -> str:
    # Explicit override wins so the idapro-free supervisor (and tests) can point
    # the registry anywhere without an IDA install.
    override = os.environ.get("IDA_MCP_USER_DIR")
    if override:
        return override
    if sys.platform == "win32":
        ...
        # Fallback when APPDATA is absent (e.g. service accounts).
        return os.path.join(os.path.expanduser("~"), "AppData", "Roaming", "Hex-Rays", "IDA Pro")
...
def find_instance_for_session(session_id: str) -> InstanceInfo | None:
    """N-copies: return the live instance owning `session_id`, or None.

    Unambiguous even when multiple workers share one binary path, because
    session_id is unique per worker while idb_path is not.
    """
    for info in discover_instances():
