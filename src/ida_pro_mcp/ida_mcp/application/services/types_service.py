"""Application service for IDA type-system operations (api_types domain).

Holds the orchestration logic for the type tools, moved faithfully from the
flat ``api_types`` module. Behavior preservation OUTRANKS layering purity:
functions whose control flow is tightly interleaved with ``ida_typeinf`` /
``idaapi`` calls keep those calls here rather than forcing an awkward adapter
seam. The cleanly-extractable byte reads and inference primitives are reached
through ``TypesAdapter``.

Module-level ``idaapi`` / ``ida_typeinf`` imports are intentionally avoided so
this file compiles outside IDA; IDA modules are imported lazily where used.
"""
...
class TypesService:
    """High-level service for IDA type-system operations."""

    def __init__(self, adapter: TypesAdapter):
    ...
    def declare_type(self, decls: "list[str] | str") -> list[dict]:
        import ida_typeinf
    ...
    def enum_upsert(self, queries) -> list[dict]:
        import idc
            ...
            try:
                ...
                for member in members:
                    ...
                    if not member_name:
                        ...
                        continue
                    try:
                        value = self._parse_enum_value(raw_value)
    ...
    def read_struct(self, queries) -> list[dict]:
        import idaapi
        import ida_typeinf
        import ida_nalt
                ...
                for member in udt_data:
                    ...
                    try:
                        if member.type.is_ptr():
                            is_64bit = self.adapter.is_64bit()
                            if is_64bit:
    ...
    def search_structs(self, filter: str, offset: int = 0, count: int = 100) -> dict:
        import ida_typeinf

        results = []
        limit = self.adapter.get_ordinal_limit()
    ...
    def _type_matches_kind(self, kind: str, tif) -> bool:
        if kind == "any":
            return True
        if kind == "udt":
            try:
                ...
                return False
        return self._type_kind(tif) == kind

    # =====================================================================
    # Type Inference & Application
    # =====================================================================

    def type_query(self, queries) -> list[dict]:
        import ida_typeinf
        ...
        for query in queries:
            ...
            for row in catalog:
                ...
                if not isinstance(tif, ida_typeinf.tinfo_t):
                    continue
                if not self._type_matches_kind(kind, tif):
                    continue
                filtered.append(row)
    ...
    def type_inspect(self, queries) -> list[dict]:
        import ida_typeinf
    ...
    @staticmethod
    def _parse_type_tinfo(type_text: str):
        import ida_typeinf
    ...
    @staticmethod
    def _parse_function_tinfo(signature_text: str):
        import ida_typeinf
    ...
    def _infer_type_edit_kind(self, edit: dict) -> str:
        import idaapi
        import ida_typeinf
        import ida_frame
        ...
        if "addr" in edit and "name" in edit and self._resolve_type_text(edit):
    ...
    def _apply_type_edit(self, edit: dict) -> dict:
        import idaapi
        import ida_typeinf
        import ida_hexrays
        import ida_frame
        ...
        try:
            ...
            type_text = self._resolve_type_text(edit)
    ...
    def set_type(self, edits) -> list[dict]:
        normalized_edits = normalize_dict_list(edits, self._parse_addr_type_shorthand)
        return [self._apply_type_edit(edit) for edit in normalized_edits]

    def type_apply_batch(self, batch) -> dict:
        if isinstance(batch, dict) and "edits" in batch:
            normalized_edits = normalize_dict_list(
                batch.get("edits", []), self._parse_addr_type_shorthand
            )
            stop_on_error = bool(batch.get("stop_on_error", False))
        else:
            normalized_edits = normalize_dict_list(batch, self._parse_addr_type_shorthand)
            stop_on_error = False

        results: list[dict] = []
        for edit in normalized_edits:
            result = self._apply_type_edit(edit)
    ...
    def infer_types(self, addrs: "list[str] | str") -> list[dict]:
        import ida_typeinf
        import ida_nalt
        import ida_bytes
        ...
        for addr in addrs:
            try:
                ...
                if self.adapter.guess_tinfo(tif, ea):
                    results.append(
                        {
                            "addr": addr,
