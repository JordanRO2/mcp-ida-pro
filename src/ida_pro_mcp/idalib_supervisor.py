...
class IdalibSupervisor:
    def __init__(
        ...
        *,
        ...
        # N-copies: one binary path may back MANY concurrent sessions (distinct
        # worker processes / private DB copies), so this is path -> {session_id}.
        # It is used only for observability (idb_list grouping / a future
        # annotation-merge tool), NEVER for routing or auto-dedup.
        self.path_to_session: dict[str, set[str]] = {}
    ...
    def _register_session_locked(self, session: WorkerSession, resolved_path: str) -> None:
        ...
        for candidate in self._candidate_idb_paths(resolved_path):
            self.path_to_session.setdefault(candidate, set()).add(session.session_id)
    ...
    def _unregister_session_locked(self, session_id: str) -> WorkerSession | None:
        ...
        # Discard ONLY this session's id from each path's set; drop keys that
        # become empty. A sibling copy sharing the same path is untouched.
        for path_key in list(self.path_to_session):
            ids = self.path_to_session.get(path_key)
            if ids is None:
                continue
    ...
    def _launch_gui_and_adopt(self, resolved_path: str, session_id: str) -> WorkerSession:
        ...
        with self._lock:
            # GUI is naturally single-instance-per-path: reuse a live GUI-backed
            # session for this path if one exists (path_to_session is a set of
            # ids under N-copies, so scan it and drop stale entries).
            for existing_id in sorted(self.path_to_session.get(self._path_key(resolved_path), set())):
                existing_session = self.sessions.get(existing_id)
                ...
                self._unregister_session_locked(existing_id)
    ...
    def open_session(
        ...
        *,
        ...
        with self._lock:
            # N-copies SHARE opt-in: an explicit, still-reachable session_id
            # means "attach to that live worker/IDB" so agents can collaborate
            # on ONE database. This is the ONLY way opens are deduplicated.
            if session_id is not None:
                shared = self.sessions.get(session_id)
                if shared is not None and self._session_is_reachable(shared):
                    shared.last_accessed = datetime.now()
                    return shared
                if shared is not None:
                    # Stale id: drop it and reuse the id for a fresh worker.
                    self._unregister_session_locked(session_id)
            else:
                # Default: ALWAYS mint a fresh unique id, so a second open of
                # the SAME binary spawns a SEPARATE worker/IDB (N-copies). The
                # stem prefix keeps ids human-traceable across duplicates.
                session_id = f"{Path(resolved).stem}-{uuid.uuid4().hex[:8]}"
            ...
            if mode in ("prefer_gui", "force_gui"):
                ...
                break_for_launch = False
                # N-copies: headless opens NEVER auto-adopt an existing worker
                # by path — that would collapse duplicates of one binary into a
                # single worker and destroy parallelism. Sharing is explicit
                # via a reused session_id (handled at the top of this method).
        ...
        try:
            ...
            self._terminate_worker(worker)
            # N-copies: the worker opened a PRIVATE copy under its own scratch
            # dir, so there is nothing to clean next to the shared original —
            # and deleting the original's neighbours could corrupt a live
            # sibling copy. The reaped worker's scratch dir is disposable.
            raise RuntimeError(
        ...
        with self._lock:
            # N-copies: the unique session_id means there is NO path-collision
            # to reconcile (two copies of one binary are two valid sessions).
            # The only race is on the id itself, if a caller passed the SAME
            # explicit id concurrently: the existing reachable session wins
            # (share) and this freshly-opened worker is discarded; otherwise we
            # register this worker as the session.
            existing_by_id = self.sessions.get(session_id)
            if existing_by_id is not None and self._session_is_reachable(existing_by_id):
                existing_by_id.last_accessed = datetime.now()
                share_session = existing_by_id
            else:
...
def idb_open(
    ...
    preferred_session_id: Annotated[
        str,
        "Leave empty to open a NEW parallel copy (its own worker/IDB) even if "
        "the binary is already open. Pass an EXISTING session id to SHARE that "
        "live worker/IDB instead (collaborate on one database).",
    ] = "",
) -> IdalibOpenResult:
    """Open a binary and warm it up, returning a session id to route tools to.

    N-copies: each call with an empty preferred_session_id spawns a SEPARATE
    worker process with its own private copy of the binary, so multiple agents
    can analyze the SAME binary in parallel (annotations diverge per copy).
    Pass an existing session id to share one worker/IDB instead.
    """
